...
uint32_t prg_hash_INES(const ines_cart_t *cart) {
	return ines_crc32(cart->prg_rom, prg_size_INES(cart) * 0x4000, 0);
}
...
uint32_t chr_hash_INES(const ines_cart_t *cart) {
	return ines_crc32(cart->chr_rom, chr_size_INES(cart) * 0x2000, 0);
}
...
uint32_t rom_hash_INES(const ines_cart_t *cart) {
	...
	const uint32_t hash1 = cart->trainer ? ines_crc32(cart->trainer, 512, 0)                              : 0;
	const uint32_t hash2 = cart->prg_rom ? ines_crc32(cart->prg_rom, prg_size_INES(cart) * 0x4000, hash1) : hash1;
	const uint32_t hash3 = cart->prg_rom ? ines_crc32(cart->chr_rom, chr_size_INES(cart) * 0x2000, hash2) : hash2;
